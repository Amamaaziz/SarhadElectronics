  ...
  try {
    ...
    if (isDbLive) {
      try {
        ...
        console.warn('⚠️ Firestore createCategory error, using local persistent storage:', err);
      }
    }
  ...
  try {
    ...
    if (isDbLive) {
      try {
        ...
        const memIdx = memoryCategories.findIndex((c) => c.id === catData.id);
        if (memIdx !== -1) {
          memoryCategories.splice(memIdx, 1);
          saveCategoriesToDisk();
        }
        ...
        console.warn('⚠️ Firestore deleteCategory error:', err);
      }
    }
